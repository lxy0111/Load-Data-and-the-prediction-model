...
data_path = r'/home/lthpc/桌面/battery_formation/battery_formation_data/250mAh/cycling/3-3.xls'
...
Cycle_end  = None
# find the column number for different measurements
...
for i in range(len(Current_ALL_Discharge_process)-1):
    # Linear interpolation of charge and discharge data to the sanme time step of 500.
    Time_Steps_discharge = len(Time_ALL_Discharge_process[i+1])
    old_x_discharge = []
    for j in range(Time_Steps_discharge):
        old_x_discharge.append(j)
    x_new_discharge = np.linspace(0, Time_Steps_discharge - 1, 500, endpoint=True)

    Current_ALL_Discharge_process_new = np.interp(x_new_discharge,old_x_discharge,
                                                  Current_ALL_Discharge_process[i+1])
    Voltage_ALL_Discharge_process_new = np.interp(x_new_discharge, old_x_discharge,
                                                  Voltage_ALL_Discharge_process[i + 1])
    Capacity_ALL_Discharge_process_new = np.interp(x_new_discharge, old_x_discharge,
                                                  Capacity_ALL_Discharge_process[i + 1])
    Time_ALL_Discharge_process_new = np.interp(x_new_discharge, old_x_discharge,
                                                  Time_ALL_Discharge_process[i + 1])

    Time_Steps_charge = len(Time_ALL_Charge_process[i + 1])
    old_x_charge = []
    for j in range(Time_Steps_charge):
        old_x_charge.append(j)
    x_new_charge = np.linspace(0, Time_Steps_charge - 1, 500, endpoint=True)

    Current_ALL_Charge_process_new = np.interp(x_new_charge, old_x_charge,
                                                  Current_ALL_Charge_process[i + 1])
    Voltage_ALL_Charge_process_new = np.interp(x_new_charge, old_x_charge,
                                                  Voltage_ALL_Charge_process[i + 1])
    Capacity_ALL_Charge_process_new = np.interp(x_new_charge, old_x_charge,
                                                   Capacity_ALL_Charge_process[i + 1])
    Time_ALL_Charge_process_new = np.interp(x_new_charge, old_x_charge,
                                               Time_ALL_Charge_process[i + 1])
    # save the cycling data 
    summary = {'I_charge': Current_ALL_Charge_process_new,
                   'I_discharge': Current_ALL_Discharge_process_new,
                   'V_charge': Voltage_ALL_Charge_process_new,
                   'V_discharge': Voltage_ALL_Discharge_process_new,
               'Qc_charge': Capacity_ALL_Charge_process_new,
                   'Qd_discharge': Capacity_ALL_Discharge_process_new,
               't_charge': Time_ALL_Charge_process_new,
                   't_discharge': Time_ALL_Discharge_process_new}
    with open('/home/lthpc/桌面/battery_formation/battery_formation_data/250mAh/cycling/3-3-cycling.csv', 'a+', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(summary['I_charge'])
        writer.writerow(summary['I_discharge'])
        writer.writerow(summary['V_charge'])
        writer.writerow(summary['V_discharge'])
        writer.writerow(summary['Qc_charge'])
        writer.writerow(summary['Qd_discharge'])
        writer.writerow(summary['t_charge'])
        writer.writerow(summary['t_discharge'])
'''
#Current
...
for i in range(1):
    ax1.plot(Time_ALL_Discharge_process[i],Capacity_ALL_Discharge_process[i])
